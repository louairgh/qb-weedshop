...
local function removeBlips()
  local blip = GetFirstBlipInfoId(140)
  repeat RemoveBlip(blip); blip = GetNextBlipInfoId(140) until not DoesBlipExist(blip)
end
...
AddEventHandler('QBCore:Client:OnPlayerUnload', function()
  for i = 1, #Config.Vitrines do
    if Config.Vitrines[i].isBusy then
      TriggerServerEvent('qb-weedshop:server:SetVitrineState', false, i)
    end
  end
  removeBlips()
end)
...
AddEventHandler('onResourceStart', function(resource)
  ...
  TriggerServerEvent('qb-weedshop:server:StoreHit', 'all', false)
  createBlips()
end)
...
AddEventHandler('qb-weedshop:client:SmashCase', function(case)
  QBCore.Functions.TriggerCallback('qb-weedshop:server:GetCops', function(cops)
    if not checkTime(Config.weedshopHours.range.open, Config.weedshopHours.range.close) then
        ...
        if cops >= Config.RequiredCops then
          if isStoreHit(case, false) or isStoreHacked() then
            ...
            QBCore.Functions.Progressbar('smash_vitrine', Lang:t('info.smashing_progress'), Config.WhitelistedWeapons[pedWeapon].timeOut, false, true, {
              ...
              TriggerServerEvent('qb-weedshop:server:VitrineReward', case)
              TriggerServerEvent('qb-weedshop:server:SetTimeout', case)
              if not secondAlarm then 
                if Config.HackEffect == 'disableAlarm' and not isStoreHacked() or Config.HackEffect == 'disableCam' then
                  ...
                  elseif Config.Dispatch == 'ps' then
                    exports['ps-dispatch']:WeedShopHeist()
              ...
              TriggerServerEvent('qb-weedshop:server:SetVitrineState', 'isBusy', false, case)
              smashing = false
              TaskPlayAnim(ped, animDict, 'exit', 3.0, 3.0, -1, 2, 0, 0, 0, 0)
            end)
            TriggerServerEvent('qb-weedshop:server:SetVitrineState', 'isBusy', true, case)
...
AddEventHandler('qb-weedshop:client:Thermite', function(store)
  local AlertChance = randomNum(1, 100)
  if checkTime(Config.weedshopHours.alertmorn.start, Config.weedshopHours.alertmorn.fin) or checkTime(Config.weedshopHours.alertnight.start, Config.weedshopHours.alertnight.fin) then
        ...
        if dist <= 1.5 then
          if QBCore.Functions.HasItem(Config.DoorItem) then
            ...
            exports['ps-ui']:Thermite(function(success) -- success
              if success then
                TriggerServerEvent('qb-weedshop:server:StoreHit', store, true)    
                ...
                Wait(4000)
                TriggerServerEvent('qb-weedshop:server:RemoveDoorItem')
                ...
                TriggerEvent('qb-weedshop:client:HackSuccess', store)
...
AddEventHandler('qb-weedshop:client:HackSecurity', function()
  QBCore.Functions.TriggerCallback('qb-weedshop:server:GetCops', function(cops)
    if not checkTime(Config.weedshopHours.range.open, Config.weedshopHours.range.close) then
      if Config.Skills.enabled then
          ...
          if QBCore.Functions.HasItem(Config.HackItem) then
            ...
            exports['ps-ui']:VarHack(function(success)
              if success then
                ...
                hacking = false
                TriggerServerEvent('qb-weedshop:server:StoreHit', 'all', true)
                Wait(250)
                StopAnimTask(ped, animDict, anim, 8.0)
                DeleteEntity(tab)
                TriggerEvent('qb-weedshop:client:HackSuccess')
              else
                hacking = false
...
AddEventHandler('qb-weedshop:client:HackSuccess', function(store)
    ...
    if isStoreHit(store, true)  and not isStoreHacked() then
      if not Config.OneStore then
        ...
        if Config.AutoLock then
          TriggerServerEvent('qb-weedshop:server:ToggleDoorlocks', store, false, false)
          locked = false
        ...
        if Config.AutoLock then TriggerServerEvent('qb-weedshop:server:ToggleDoorlocks', store, false, false) end
        locked = false
      ...
      if Config.AutoLock and not checkTime(Config.weedshopHours.range.open, Config.weedshopHours.range.close) then
        TriggerServerEvent('qb-weedshop:server:ToggleDoorlocks', store, true, false)
        locked = true
      end
      TriggerServerEvent('qb-weedshop:server:StoreHit', store, false)
    else
      if not Config.OneStore then 
        QBCore.Functions.Notify(Lang:t('success.hacked_threestore'), 'success')
        if Config.AutoLock then TriggerServerEvent('qb-weedshop:server:ToggleDoorlocks', nil, false, true) end
        locked = false
        Wait(Config.Cooldown)
        if Config.AutoLock and not checkTime(Config.weedshopHours.range.open, Config.weedshopHours.range.close) then
          locked = true
          TriggerServerEvent('qb-weedshop:server:ToggleDoorlocks', nil, true, true)
        end
        TriggerServerEvent('qb-weedshop:server:StoreHit', 'all', false)
      else
...
RegisterNetEvent('qb-weedshop:client:SetVitrineState', function(stateType, state, k)
...
RegisterNetEvent('qb-weedshop:client:StoreHit', function(storeIndex, isHit)
      ...
      options = {
        {
          ...
          action = function()
            if validWeapon() then
              TriggerEvent('qb-weedshop:client:SmashCase', k)
            else
      ...
      options = {
        {
          ...
          item = 'thermite',
          action = function()
            TriggerEvent('qb-weedshop:client:Thermite', k)
          end
        }
      },
      ...
      options = {
        {
          ...
          action = function()
            if GetSelectedPedWeapon(ped) == `WEAPON_UNARMED` then
              ...
              if validWeapon() then
                TriggerEvent('qb-weedshop:client:SmashCase', i)
              else
                QBCore.Functions.Notify(Lang:t('error.wrong_weapon'), 'error')
              end
            end
    ...
    options = {
      {
        ...
        action = function()
          TriggerEvent('qb-weedshop:client:Thermite', 1)
        end
      }
    },
    distance = 2.5 -- This is the distance for you to be at for the target to turn blue, this is in GTA units and has to be a float value
...
}, 
{
  options = {
    ...
    action = function()
        TriggerEvent('qb-weedshop:client:HackSecurity')
      end
    }
  },
  distance = 2.5 -- This is the distance for you to be at for the target to turn blue, this is in GTA units and has to be a float value
    ...
    if LocalPlayer.state.isLoggedIn then
      if not checkTime(Config.weedshopHours.range.open, Config.weedshopHours.range.close) then
        ...
        if not loopDone then
          Wait(1000)
          TriggerServerEvent('qb-weedshop:server:ToggleDoorlocks', nil, false, true)
          loopDone = true
        end
      end
    end
